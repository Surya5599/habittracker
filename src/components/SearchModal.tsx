...
export const SearchModal: React.FC<SearchModalProps> = ({
    ...
    return (
        <div
            className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[15vh] bg-scrim backdrop-blur-sm animate-in fade-in duration-200"
            onClick={onClose}
        >
            <div
                className="w-full max-w-2xl bg-surface border-3 border-edge-strong shadow-neo-lg rounded-modal overflow-hidden flex flex-col animate-in slide-in-from-top-4 duration-300"
                onClick={e => e.stopPropagation()}
            >
                {/* Header / Input */}
                <div className="flex items-center gap-3 p-3 border-b-2 border-edge-strong bg-surface-muted">
                    <Search className="text-ink-subtle" size={20} />
                    <input
                        ref={inputRef}
                        type="text"
                        value={query}
                        ...
                        className="flex-1 bg-transparent text-sm font-bold placeholder:text-ink-dim outline-none"
                    />
                    <div className="hidden sm:flex items-center gap-1">
                        <kbd className="px-2 py-1 bg-surface border border-edge-muted rounded-lg text-[10px] font-bold text-ink-muted shadow-sm">ESC</kbd>
                    </div>
                </div>
                ...
                <div
                    ref={listRef}
                    className="max-h-[60vh] overflow-y-auto p-2 space-y-1 bg-surface"
                >
                    {results.length === 0 && query.trim() && (
                        <div className="p-8 text-center text-ink-subtle font-medium">
                            No results found for "{query}"
                        </div>
                    )}

                    {results.length === 0 && !query.trim() && (
                        <div className="p-12 flex flex-col items-center justify-center text-ink-dim gap-2">
                    ...
                    {results.map((result, index) => (
                        <div
                            ...
                            className={`flex items-center gap-3 p-2 cursor-pointer border-2 transition-all ${index === selectedIndex
                                ? 'border-edge-strong bg-surface-muted shadow-neo-sm -translate-y-0.5'
                                : 'border-transparent hover:border-edge hover:bg-surface-muted'
                                }`}
                        >
                            <div className={`w-10 h-10 flex items-center justify-center border-2 border-edge-strong bg-surface shadow-sm shrink-0 ${result.type === 'habit' ? 'text-blue-500' : 'text-orange-500'
                                }`}>
                                {result.type === 'habit' ? <CheckSquare size={20} /> : <Calendar size={20} />}
                            </div>

                            <div className="flex-1 min-w-0">
                                <h4 className="font-bold text-ink-strong text-xs truncate">{result.title}</h4>
                                {result.subtitle && (
                                    <p className="text-[10px] text-ink-muted font-medium whitespace-pre-wrap mt-0.5">{result.subtitle}</p>
                                )}
                            </div>

                            {index === selectedIndex && (
                                <ArrowRight size={16} className="text-ink-strong animate-pulse" />
                            )}
                        </div>
                    ))}
                </div>
                ...
                {/* Footer Helper */}
                {results.length > 0 && (
                    <div className="px-4 py-2 border-t-2 border-edge-strong bg-surface-strong flex justify-between items-center text-[10px] font-bold text-ink-muted uppercase tracking-wider">
