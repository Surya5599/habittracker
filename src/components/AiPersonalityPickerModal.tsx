...
export const AiPersonalityPickerModal: React.FC<AiPersonalityPickerModalProps> = ({ isOpen, onSelect, themePrimary }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-3 backdrop-blur-sm">
            <div
            ...
            >
                ...
                <div className="px-4 py-3 border-b-[2px] border-black flex items-center gap-2 shrink-0">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-medium text-stone-500">Same insights, different tone. You'll be asked again tomorrow.</p>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
                    {AI_COACH_PERSONALITIES.map((p) => (
                        <button
                            ...
                            className="flex items-start gap-3 p-3 rounded-xl border-2 border-black text-left bg-white hover:bg-stone-50 transition-all duration-100 active:translate-x-[1px] active:translate-y-[1px]"
                            style={{ boxShadow: '3px 3px 0px 0px rgba(0,0,0,1)' }}
                        >
                            <img
                                src={personalityAvatarUrl(p.id)}
                                alt={p.label}
                                className="w-12 h-12 rounded-full border-2 border-black shrink-0 bg-stone-100"
                                loading="lazy"
                            />
                            <div className="min-w-0">
                                <span className="text-[12px] font-black text-stone-900">{p.label}</span>
                                <p className="text-[11px] text-stone-600 mt-1 leading-snug">{p.description}</p>
                            </div>
                        </button>
                    ))}
                </div>
